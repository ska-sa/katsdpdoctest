pipeline {
    ...
    stages {
	    ...
	    environment {
		  ...
		  echo "Test ls"
                  echo `docker run --rm -i -v jenkins_home:/var/jenkins_home -v=$(pwd):/docs sdp-docker-registry.kat.ac.za:5000/docker-base-build ls /docs`
		  echo `docker run --rm -i -v jenkins_home:/var/jenkins_home -v=$(pwd):/docs sdp-docker-registry.kat.ac.za:5000/docker-base-build ls /`
